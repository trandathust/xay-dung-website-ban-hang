...
<section id="cart_items">
    <div class="container">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="cart_product">
                                <a href=""><img src="{{$details['feature_image_path']}}" class="image-cart-70-100"
                                        alt=""></a>
                            </td>
                            ...
                            <td class="cart_price">
                                @if($details['price'] == $details['price_sale'] || $details['price_sale'] == null ||
                                $timenow > $details['end_sale'])
                            ...
                            <td class="cart_quantity" data-th="Quantity">
                                <div class="cart_quantity_button">
                                    <a class="cart_quantity_up update-cart" data-id="{{ $id }}"
                                        data-url="{{route('updatecart')}}" data-note="0"> - </a>
                                    <input class="cart_quantity_input quantity" type="text" name="quantity"
                                        value="{{$details['quantity']}}" autocomplete="off" size="2">
                                    <a class="cart_quantity_up update-cart" data-id="{{ $id }}"
                                        data-url="{{route('updatecart')}}" data-note="1"> + </a>
                                </div>
                            </td>
                            <td class="cart_total">
                                @if($details['price'] == $details['price_sale'] || $details['price_sale'] == null ||
                                $timenow > $details['end_sale'])
                                <p class="cart_total_price">{{number_format($details['price'] * $details['quantity'])}}
                                    đ</p>
                                @else
                                <del>
                                ...
                                <br>
                                <p class="cart_total_price">
                                    {{number_format($details['price_sale'] * $details['quantity'])}} đ</p>
                                @endif
                            </td>
                            <td class="cart_delete">
                                <a class="cart_quantity_delete remove-from-cart" data-url="{{route('deletecart')}}"
        ...
        <div class="register-req">
            <p>Hãy <a href=""> đăng ký tài khoản</a> để dễ dàng kiểm tra đơn đặt hàng. Hoặc bạn có thể sử dụng tài khoản
                khách để mua hàng. </p>
        </div>
        <!--/register-req-->
        @endif
                    ...
                    <div class="product-image-wrapper">
                        <div class="single-products">
                            <div class="productinfo text-center">
                                <a href="{{route('getProductDetail',['id' => $product -> id])}}"><img
                                        src="{{$product -> feature_image_path}}" class="image-check-out" alt="" /></a>
                                <h2>{{number_format($product -> price)}}</h2>
                                <p>{{$product -> name}}</p>
                                <input type="hidden" name="id" id="id" value="{{$product -> id}}" />
                                <input type="hidden" name="quantity" id="quantity" value="1" />
                                <a data-url="{{route('saveCart',['id'=> $product -> id])}}"
                                    class="btn btn-default add-to-cart btn_add_to_cart_checkout"><i
